    ...
    perSystem = mkPerSystemOption ({ pkgs, ... }:
      ...
      in {
        ...
        options.loaded-nvim' = mkOption {
          ...
          type = with types; attrsOf (submodule { options = opt; });
          default = { };
        };
      });
  };
  config = {
    perSystem = { config, lib, pkgs, ... }:
      let
        ...
        patch = cfg:
          let
            excludeFiles = flatten ((if cfg.did_load_ftplugin then
              ...
              ++ (if cfg.did_install_default_menus then
                ...
                [ ]) ++ (if cfg.skip_loading_mswin then
                  [ "runtime/mswin.vim" ]
                else
                  [ ]) ++ (if cfg.loaded_gzip then
          ...
          in cfg.package.overrideAttrs (old: {
            version = "loaded";
            postInstall = ''
              ${old.postInstall or ""}
              ${concatStringsSep "\n" postInstallCommands}
      ...
      in {
        packages = {
          ...
          (name: cfg: nameValuePair "loaded-nvim-${name}" (patch cfg))
          config.loaded-nvim');
      };
  };
}
